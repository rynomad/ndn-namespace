...
window.RTCPeerConnection = require("./adapter/adapter.js")
console.log(RTCPeerConnection)
window.pc_config = {"iceServers": [{"url": "stun:stun.5.google.com:19302"}]};
...
window.pcs = []
module.exports = function(suffixCallback, connectionInfoCallback){
  var forwarder = this;
  var suffixMade = false
  var pc = new RTCPeerConnection(window.pc_config, {optional: [{RTPDataChannels: true}]} )
  pcs.push(pc)
  var offer = {};
  offer.candidates = [];
  ...
  pc.ondatachannel = function(event) {
    ...
    connectionInfoCallback( event.channel);
  };

  window.datachannel = pc.createDataChannel("chan", {reliable: false})

  pc.onicecandidate = function (evt) {
    if (evt.candidate === null && offer.candidates.length > 0 && suffixMade === false){
      suffixMade = true;
      offers.push(offer)
      suffixCallback(createSuffix())
    }
    console.log("!!!")
